...
/// Author      : Gihed Annabi
/// Date        : 02-2026
/// Purpose     : Command to generate the best fun motorcycle route for a given origin,
///              destination or loop specification, applying the supplied scoring profile
...
public sealed class GenerateRouteCommand : ICommand<GenerateRouteResponse>
{
	...
	/// <summary>When true, ferry crossings are avoided.</summary>
	public bool AvoidFerries { get; init; }

	/// <summary>When true, motorway on-ramps and link roads are avoided.</summary>
	public bool AvoidMotorwayLinks { get; init; }

	/// <summary>Maximum total ride duration in minutes. Null = no cap. Range: [1–1,440].</summary>
	public int? MaxDurationMinutes { get; init; }

	/// <summary>Maximum total distance in metres. Null = no cap.</summary>
	...
	/// <summary>Custom only: weight for curvature/twistiness [0–1]. Required when Preset = Custom.</summary>
	public double? CurvesWeight { get; init; }

	/// <summary>Custom only: weight for scenic surroundings [0–1]. Required when Preset = Custom.</summary>
	public double? SceneryWeight { get; init; }

	/// <summary>Custom only: weight for elevation/mountain relief [0–1]. Required when Preset = Custom.</summary>
	...
	/// using roads that are different from the outbound journey via Valhalla exclude_polygons.
	/// The response ReturnLeg field will be populated. Ignored in loop mode. Default: false.
	/// </summary>
	public bool RoundTrip { get; init; }

	/// <summary>Point-to-point destination latitude. Mutually exclusive with LoopTargetDistanceMeters.</summary>
	public double? EndLatitude { get; init; }

	/// <summary>Point-to-point destination longitude. Mutually exclusive with LoopTargetDistanceMeters.</summary>
	public double? EndLongitude { get; init; }

	/// <summary>Loop mode: desired total loop distance in metres [1,000–500,000]. Mutually exclusive with EndLatitude/EndLongitude.</summary>
	public double? LoopTargetDistanceMeters { get; init; }

	/// <summary>Loop mode only: SameRoute (0) or DifferentRoute (1). Ignored in point-to-point mode.</summary>
