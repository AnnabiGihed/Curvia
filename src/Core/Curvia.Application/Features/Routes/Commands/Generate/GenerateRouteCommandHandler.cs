...
internal sealed class GenerateRouteCommandHandler : ICommandHandler<GenerateRouteCommand, GenerateRouteResponse>
{
	...
	private async Task<Result<ReturnLegDto>> BuildLoopReturnLegAsync(RoutePlan plan, ValhallaRouteResponse outboundResponse, CancellationToken cancellationToken)
	{
	...
	private async Task<Result<ReturnLegDto>> ScoreAndBuildReturnLeg(RoutePlan plan, IReadOnlyList<RouteCandidate> candidates, CancellationToken cancellationToken)
	{
		var scoreTasks = candidates.Select(c => _scoringService.ScoreAsync(c, plan, cancellationToken)).ToList();
	...
	private async Task<Result<ReturnLegDto>> BuildRoundTripReturnLegAsync(RoutePlan plan, ValhallaRouteResponse outboundResponse, CancellationToken cancellationToken)
	{
		IReadOnlyList<RouteCandidate> returnCandidates;
		...
		{
			return Result.Failure<ReturnLegDto>(new Error("Routing.Valhalla.RoundTripReturnFailed", $"Round-trip return leg generation failed: {ex.Message}"));
		}

		if (returnCandidates.Count == 0)
			return Result.Failure<ReturnLegDto>(new Error("Routing.RoundTripReturn.NoCandidates", "No return leg candidates were generated for the round trip. " + "The outbound route corridor may be covering all available roads between these two points. " + "Try relaxing AvoidHighways or MaxDetourRatio."));
