...
///              Extends AggregateRoot — permanent hazards are static reference data
///              with no domain events or aggregate invariants beyond the value-object
...
public sealed class Hazard : AggregateRoot<HazardId>
{
	...
	/// <param name="description">Optional description — may be null.</param>
	...
	/// Null or empty description is accepted — the aggregate stores null in that case.
	/// </summary>
	...
	/// <param name="hazardType">Hazard classification.</param>
	/// <param name="description">Optional human-readable description. Null or whitespace stores null.</param>
	...
	public static Result<Hazard> Create(string externalId, string source, string countryCode, double latitude, double longitude, HazardType hazardType, string? description)
	{
		...
		var hazard = new Hazard(HazardId.New(), externalIdResult.Value, sourceResult.Value, countryCodeResult.Value, positionResult.Value, hazardType, descriptionVo, now);
	...
	/// <param name="description">Updated optional description. Null or whitespace stores null.</param>
