...
/// </summary>
internal sealed class MotorcycleRepository : BaseAsyncCommandRepository<Motorcycle, MotorcycleId>, IMotorcycleRepository
{
	#region Constructor
	/// <summary>
	/// Initializes a new instance of <see cref="MotorcycleRepository"/> backed by <see cref="CurviaDbContext"/>.
	/// </summary>
	/// <param name="dbContext">EF Core database context.</param>
	public MotorcycleRepository(CurviaDbContext dbContext) : base(dbContext) { }
	#endregion

	#region IMotorcycleRepository
	/// <inheritdoc/>
	public async Task<Motorcycle?> FindDefaultAsync(UserId userId, CancellationToken cancellationToken = default)
	{
		return await DbContext.Set<Motorcycle>()
			.FirstOrDefaultAsync(m => m.UserId == userId && m.IsDefault, cancellationToken);
	}

	/// <inheritdoc/>
	public async Task ClearDefaultAsync(UserId userId, MotorcycleId exceptId, CancellationToken cancellationToken = default)
	{
	...
	/// <inheritdoc/>
	public async Task<IReadOnlyList<Motorcycle>> GetByUserIdAsync(UserId userId, CancellationToken cancellationToken = default)
	{
		return await DbContext.Set<Motorcycle>()
	...
	/// <inheritdoc/>
	public async Task<Motorcycle?> FindByIdAndUserAsync(MotorcycleId id, UserId userId, CancellationToken cancellationToken = default)
	{
		return await DbContext.Set<Motorcycle>()
